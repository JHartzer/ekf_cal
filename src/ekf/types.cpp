...
State & operator+=(State & l_state, const Eigen::VectorXd & r_vector)
{
  ...
  for (auto & imu_iter : l_state.imu_states) {
    if (imu_iter.second.GetIsExtrinsic()) {
      ...
      imu_iter.second.ang_i_to_b =
        imu_iter.second.ang_i_to_b * RotVecToQuat(r_vector.segment<3>(idx + 3));
      idx += g_imu_extrinsic_state_size;
    }
    if (imu_iter.second.GetIsIntrinsic()) {
      imu_iter.second.acc_bias += r_vector.segment<3>(idx + 0);
      imu_iter.second.omg_bias += r_vector.segment<3>(idx + 3);
      idx += g_imu_intrinsic_state_size;
    }
  }

  for (auto & gps_iter : l_state.gps_states) {
    if (gps_iter.second.GetIsExtrinsic()) {
      gps_iter.second.pos_a_in_b += r_vector.segment<3>(idx);
      idx += g_gps_extrinsic_state_size;
    }
  }

  for (auto & cam_iter : l_state.cam_states) {
    if (cam_iter.second.GetIsExtrinsic()) {
      cam_iter.second.pos_c_in_b += r_vector.segment<3>(idx + 0);
      cam_iter.second.ang_c_to_b =
        cam_iter.second.ang_c_to_b * RotVecToQuat(r_vector.segment<3>(idx + 3));
      idx += g_cam_extrinsic_state_size;
    }
  }

  for (auto & fid_iter : l_state.fid_states) {
    if (fid_iter.second.GetIsExtrinsic()) {
      fid_iter.second.pos_f_in_l += r_vector.segment<3>(idx);
      fid_iter.second.ang_f_to_l =
        fid_iter.second.ang_f_to_l * RotVecToQuat(r_vector.segment<3>(idx + 3));
      idx += g_fid_extrinsic_state_size;
    }
  }
  ...
  for (auto & aug_iter : l_state.aug_states) {
    unsigned int aug_id = aug_iter.first;
    for (unsigned int i = 0; i < l_state.aug_states[aug_id].size(); ++i) {
      l_state.aug_states[aug_id][i].pos_b_in_l += r_vector.segment<3>(idx + 0);
      l_state.aug_states[aug_id][i].ang_b_to_l = l_state.aug_states[aug_id][i].ang_b_to_l *
        RotVecToQuat(r_vector.segment<3>(idx + 3));
      idx += g_aug_state_size;
    }
  }
...
ImuState & operator+=(ImuState & l_imu_state, const Eigen::VectorXd & r_vector)
{
  ...
  if (l_imu_state.GetIsExtrinsic()) {
    l_imu_state.pos_i_in_b += r_vector.segment<3>(idx + 0);
    l_imu_state.ang_i_to_b = l_imu_state.ang_i_to_b * RotVecToQuat(
      r_vector.segment<3>(idx + 3));
    idx += g_imu_extrinsic_state_size;
  }
  if (l_imu_state.GetIsIntrinsic()) {
    l_imu_state.acc_bias += r_vector.segment<3>(idx + 0);
    l_imu_state.omg_bias += r_vector.segment<3>(idx + 3);
    idx += g_imu_intrinsic_state_size;
  }
  ...
  std::map<unsigned int, ImuState> & l_imu_state, const Eigen::VectorXd & r_vector)
{
  ...
  for (auto & imu_iter : l_imu_state) {
    ...
    if (l_imu_state[imu_id].GetIsExtrinsic()) {
      l_imu_state[imu_id].pos_i_in_b += r_vector.segment<3>(idx + 0);
      l_imu_state[imu_id].ang_i_to_b = l_imu_state[imu_id].ang_i_to_b * RotVecToQuat(
        r_vector.segment<3>(idx + 3));
      idx += g_imu_extrinsic_state_size;
    }
    if (l_imu_state[imu_id].GetIsIntrinsic()) {
      l_imu_state[imu_id].acc_bias += r_vector.segment<3>(idx + 0);
      l_imu_state[imu_id].omg_bias += r_vector.segment<3>(idx + 3);
      idx += g_imu_intrinsic_state_size;
    }
  }
  ...
  std::map<unsigned int, CamState> & l_cam_state, const Eigen::VectorXd & r_vector)
{
  ...
  for (auto & cam_iter : l_cam_state) {
    unsigned int cam_id = cam_iter.first;
    l_cam_state[cam_id].pos_c_in_b += r_vector.segment<3>(idx + 0);
    l_cam_state[cam_id].ang_c_to_b =
      l_cam_state[cam_id].ang_c_to_b * RotVecToQuat(r_vector.segment<3>(idx + 3));
  }
  ...
  std::vector<AugState> & l_aug_state, const Eigen::VectorXd & r_vector)
{
  ...
  for (auto & aug_iter : l_aug_state) {
    aug_iter.pos_b_in_l += r_vector.segment<3>(idx + 0);
    aug_iter.ang_b_to_l = aug_iter.ang_b_to_l * RotVecToQuat(r_vector.segment<3>(idx + 3));
    idx += g_aug_state_size;
  }
...
Eigen::VectorXd ImuState::ToVector() const
{
  ...
  if (is_extrinsic) {
    out_vec.segment<3>(idx + 0) = pos_i_in_b;
    out_vec.segment<3>(idx + 3) = QuatToRotVec(ang_i_to_b);
    idx += g_imu_extrinsic_state_size;
  }
  if (is_intrinsic) {
    out_vec.segment<3>(idx + 0) = acc_bias;
    out_vec.segment<3>(idx + 3) = omg_bias;
    idx += g_imu_intrinsic_state_size;
  }
...
Eigen::VectorXd State::ToVector() const
{
  ...
  for (auto const & imu_iter : imu_states) {
    if (imu_iter.second.GetIsExtrinsic() || imu_iter.second.GetIsExtrinsic()) {
      ...
      out_vec.segment(idx, temp_vec.size()) = temp_vec;
      idx += static_cast<unsigned int>(temp_vec.size());
    }
  }

  for (auto const & gps_iter : gps_states) {
    if (gps_iter.second.GetIsExtrinsic()) {
      out_vec.segment<g_gps_extrinsic_state_size>(idx) = gps_iter.second.pos_a_in_b;
      idx += g_gps_extrinsic_state_size;
    }
  }
  ...
  for (auto const & cam_iter : cam_states) {
    if (cam_iter.second.GetIsExtrinsic()) {
      out_vec.segment(idx, g_cam_extrinsic_state_size) = cam_iter.second.ToVector();
      idx += g_cam_extrinsic_state_size;
    }
  }
  ...
  for (auto const & fid_iter : fid_states) {
    if (fid_iter.second.GetIsExtrinsic()) {
      out_vec.segment<g_fid_extrinsic_state_size>(idx + 0) = fid_iter.second.ToVector();
      idx += g_fid_extrinsic_state_size;
    }
  }
  ...
  for (auto const & aug_iter : aug_states) {
    for (unsigned int i = 0; i < aug_iter.second.size(); ++i) {
      out_vec.segment<g_aug_state_size>(idx) = aug_iter.second[i].ToVector();
      idx += g_aug_state_size;
    }
  }
...
void State::SetState(const Eigen::VectorXd & state)
{
  ...
  for (auto & imu_iter : imu_states) {
    if (imu_iter.second.GetIsExtrinsic()) {
      imu_iter.second.SetExtrinsicState(state.segment<g_imu_extrinsic_state_size>(idx));
      idx += g_imu_extrinsic_state_size;
    }
    if (imu_iter.second.GetIsIntrinsic()) {
      imu_iter.second.SetIntrinsicState(state.segment<g_imu_intrinsic_state_size>(idx));
      idx += g_imu_intrinsic_state_size;
    }
  }
  ...
  for (auto & gps_iter : gps_states) {
    if (gps_iter.second.GetIsExtrinsic()) {
      gps_iter.second.pos_a_in_b = state.segment<g_gps_extrinsic_state_size>(idx);
      idx += g_gps_extrinsic_state_size;
    }
  }
  ...
  for (auto & cam_iter : cam_states) {
    if (cam_iter.second.GetIsExtrinsic()) {
      cam_iter.second.SetState(state.segment<g_cam_extrinsic_state_size>(idx));
      idx += g_cam_extrinsic_state_size;
    }
  }
  ...
  for (auto & fid_iter : fid_states) {
    if (fid_iter.second.GetIsExtrinsic()) {
      fid_iter.second.SetState(state.segment<g_fid_extrinsic_state_size>(idx));
      idx += g_fid_extrinsic_state_size;
    }
  }
}
