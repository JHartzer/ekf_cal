...
TEST(test_ekf_types, state_plus_equals_vector) {
  ...
  state_vector.segment<3>(9) = Eigen::Vector3d::Zero();
  ...
  state_vector.segment<3>(21) = Eigen::Vector3d::Zero();
  ...
  state_vector.segment<3>(36) = Eigen::Vector3d::Zero();
  // Fiducial States
  state_vector.segment<3>(39) = Eigen::Vector3d::Ones() * 11.0;
  state_vector.segment<3>(42) = Eigen::Vector3d::Zero();
  // Augmented States
  state_vector.segment<3>(45) = Eigen::Vector3d::Ones() * 12.0;
  state_vector.segment<3>(48) = Eigen::Vector3d::Zero();
...
TEST(test_ekf_types, state_to_vector) {
  ...
  EXPECT_EQ(state_vector(21), 0.0);
  EXPECT_EQ(state_vector(22), 0.0);
  EXPECT_EQ(state_vector(23), 0.0);
  ...
  EXPECT_EQ(state_vector(36), 0.0);
  EXPECT_EQ(state_vector(37), 0.0);
  EXPECT_EQ(state_vector(38), 0.0);

  EXPECT_EQ(state_vector(39), 11.0);
  EXPECT_EQ(state_vector(40), 11.0);
  EXPECT_EQ(state_vector(41), 11.0);

  EXPECT_EQ(state_vector(42), 0.0);
  EXPECT_EQ(state_vector(43), 0.0);
  EXPECT_EQ(state_vector(44), 0.0);

  EXPECT_EQ(state_vector(45), 12.0);
  EXPECT_EQ(state_vector(46), 12.0);
  EXPECT_EQ(state_vector(47), 12.0);

  EXPECT_EQ(state_vector(48), 0.0);
  EXPECT_EQ(state_vector(49), 0.0);
  EXPECT_EQ(state_vector(50), 0.0);
}
...
TEST(test_ekf_types, state_get_state_size) {
  ...
  expected_size += g_imu_extrinsic_state_size + g_imu_intrinsic_state_size;
  EXPECT_EQ(state.GetStateSize(), expected_size);
  ...
  expected_size += g_cam_extrinsic_state_size;
  EXPECT_EQ(state.GetStateSize(), expected_size);

  AugState aug_state;
  state.aug_states[0].push_back(aug_state);

  expected_size += g_aug_state_size;
  EXPECT_EQ(state.GetStateSize(), expected_size);
}
...
TEST(test_ekf_types, set_body_state) {
  ...
  body_state_vec.segment<3>(12) = Eigen::Vector3d::Ones() * 4.0;
  body_state_vec.segment<3>(15) = Eigen::Vector3d::Ones() * 5.0;
